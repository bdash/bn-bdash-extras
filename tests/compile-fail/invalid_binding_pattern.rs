...
fn test_invalid_binding_pattern<'func, M, F>(instr: LowLevelILInstruction<'func, M, F>) -> Option<&'static str>
where
    ...
    LowLevelILExpression<'func, M, F, ValueExpr>: ExpressionHandler<'func, M, F>,
{
    let result = match_instr! {
        instr,
        // This should generate an error because @ binding with wildcard is invalid
        binding @ _ => Some("invalid"),
        _ => None,
    };
    result
}
