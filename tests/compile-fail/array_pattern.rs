...
fn test_array_pattern<'func, M, F>(instr: LowLevelILInstruction<'func, M, F>) -> Option<&'static str>
where
    ...
    LowLevelILExpression<'func, M, F, ValueExpr>: ExpressionHandler<'func, M, F>,
{
    let result = match_instr! {
        instr,
        // This should generate an error because array patterns are not supported
        [a, b] => Some("array"),
        _ => None,
    };
    result
