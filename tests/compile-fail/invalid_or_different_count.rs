...
fn test_invalid_or_different_count<'func, M, F>(instr: LowLevelILInstruction<'func, M, F>) -> Option<&'static str>
where
    ...
    LowLevelILExpression<'func, M, F, ValueExpr>: ExpressionHandler<'func, M, F>,
{
    let result = match_instr! {
        instr,
        // This should generate an error because the alternatives bind different numbers of variables
        Call(target) | TailCall(_) => {
            let _ = target;
            Some("invalid")
        },
        _ => None,
