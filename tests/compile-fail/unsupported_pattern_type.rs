...
fn test_unsupported_pattern_type<'func, M, F>(instr: LowLevelILInstruction<'func, M, F>) -> Option<&'static str>
where
    ...
    LowLevelILExpression<'func, M, F, ValueExpr>: ExpressionHandler<'func, M, F>,
{
    let result = match_instr! {
        instr,
        // This should generate an error because tuple patterns are not supported
        (a, b, c) => Some("unsupported"),
        _ => None,
    };
    result
}
